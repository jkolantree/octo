...
function execution(activity, status) {
  ...
  return {
    ...
    version: ran ? "0.3.0-alpha.13" : null,
...
function validRecord() {
  return {
    ...
    draft: true,
    protocol: { version: "0.3.0-alpha.13", sha256: HASHES.protocol },
...
test("ran non-model execution versions bind once in an output that the report references", () => {
  ...
  assert.equal(reportBytes.includes(Buffer.from("0.3.0a13", "utf8")), false);
