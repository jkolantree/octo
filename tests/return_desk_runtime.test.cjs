...
function execution(activity, status) {
  ...
  return {
    ...
    version: ran ? "0.3.0-alpha.12" : null,
...
function validRecord() {
  return {
    ...
    draft: true,
    protocol: { version: "0.3.0-alpha.12", sha256: HASHES.protocol },
...
test("ran non-model execution versions bind once in an output that the report references", () => {
  ...
  assert.equal(reportBytes.includes(Buffer.from("0.3.0a12", "utf8")), false);
