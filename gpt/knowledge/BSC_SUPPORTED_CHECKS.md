...
This theorem depends on the field and finite-dimensional splitting hypotheses. The implementation does not claim the same equivalence over arbitrary rings. See [Exact Derived Holonomy](https://github.com/jkolantree/octo/blob/v0.3.0-alpha.16/docs/DERIVED_HOLONOMY.md) for the executable contract.
...
The current public contract is [`derived-holonomy-v0.2.schema.json`](https://github.com/jkolantree/octo/blob/v0.3.0-alpha.16/schemas/derived-holonomy-v0.2.schema.json). Runtime validation dispatches legacy `holonomy_version: 0.1.0` records to the immutable [`derived-holonomy-v0.1.schema.json`](https://github.com/jkolantree/octo/blob/v0.3.0-alpha.16/schemas/derived-holonomy-v0.1.schema.json), so new v0.2 fields cannot be smuggled into the older closed contract and silently ignored. The v0.2 relation is discriminated by mode: `observation_projection` and `kernel_inclusion` are forbidden on `strict` and `derived`, preventing stronger-looking but unchecked declarations. The preserved research packet and its epistemic limits are under [`research/derived-witnessed-descent/`](https://github.com/jkolantree/octo/blob/v0.3.0-alpha.16/research/derived-witnessed-descent/README.md).
