...
- `docs/SCHEMA.md` — SHA-256 `ff7534c9b6404e9afe0a14a5108a37899913db5ac91cea30ae8ca9d2924ba3ea`
- `docs/AUDIT_RETURN_DESK.md` — SHA-256 `e9cdbf81f22b320db96e7ffe48819b9be9392691eecf92295a9f3bde28c0a3b0`
...
The machine-readable contract is [schemas/claim-manifest-v0.3.schema.json](https://github.com/jkolantree/octo/blob/main/schemas/claim-manifest-v0.3.schema.json). The schema and linter are structural tools, not truth certificates.
...
Research verdict, evidence maturity, execution status, deployment status, fatal-gate state, and any CLI decision are independent; see [STATUS_MODEL.md](https://github.com/jkolantree/octo/blob/main/docs/STATUS_MODEL.md). The claim manifest records only the coordinates in its released contract. A separate execution ledger must not be fabricated as manifest evidence. In particular:
...
[`derived-holonomy-v0.1.schema.json`](https://github.com/jkolantree/octo/blob/main/schemas/derived-holonomy-v0.1.schema.json).
...
The `v0.3.0-alpha.7` Custom GPT package did not enlarge the claim-manifest schema or the Python checker's authority. Its exact controller and Knowledge package completed the authenticated 27-case Preview gate recorded in [CUSTOM_GPT_STATUS.md](https://github.com/jkolantree/octo/blob/main/docs/CUSTOM_GPT_STATUS.md). Uploads go through ChatGPT, and no GPT Action or hosted BSC API is included.

The alpha.8 development line adds the separate closed [`audit-return-v0.1.schema.json`](https://github.com/jkolantree/octo/blob/main/schemas/audit-return-v0.1.schema.json). It describes a draft, non-admissive returned-audit envelope and does not enlarge the claim-manifest schema or grant the checker truth, proof, citation, execution-authentication, evidence-admission, or deployment authority. See [AUDIT_RETURN_DESK.md](https://github.com/jkolantree/octo/blob/main/docs/AUDIT_RETURN_DESK.md).
...
The closed schema is [`schemas/audit-return-v0.1.schema.json`](https://github.com/jkolantree/octo/blob/main/schemas/audit-return-v0.1.schema.json). The canonical production rules are in the [LLM Audit Packet](https://github.com/jkolantree/octo/blob/main/BSC_AUDIT_LLM_PACKET.md). Generated model output is still untrusted input to the Desk.
