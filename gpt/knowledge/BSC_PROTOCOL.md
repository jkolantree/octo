...
When requested or required by the selected depth, emit after the human report a JSON block named `claim_manifest.json` conforming to manifest version `0.3.0`. Use the released [schema](https://github.com/jkolantree/octo/blob/v0.3.0-alpha.6/schemas/claim-manifest-v0.3.schema.json) when available. Otherwise omit the machine-readable block rather than emitting an empty or misleading record.
