...
def arithmetic_trace_findings(raw: dict[str, Any]) -> list[Finding]:
    ...
    required = {"self_adjointness", "trace_class_resolvent", "exact_prime_increment", "joint_trace_norm_cauchy", "atomic_rigidity"}
...
def recovery_findings(raw: dict[str, Any]) -> list[Finding]:
    config = raw.get("domain_checks", {}).get("global_recovery")
    if config is None:
        return []
    if not isinstance(config, dict):
    ...
    findings: list[Finding] = []
    if config.get("local_nondegenerate") and config.get("claims_global_recovery"):
        missing = [name for name in ("fiber_unique", "boundary_complete") if not config.get(name)]
        if missing:
            findings.append(Finding(Severity.BLOCKED, "LOCAL_GLOBAL_PROMOTION_FAILURE", "domain_checks.global_recovery", "local nondegeneracy does not certify global recoverability", witness=missing, repair="supply global fiber and boundary/properness certificates"))
    return findings
