...
- Use the compiler as the single artifact source of truth. Capture the session-reported runtime once; freeze request, source, and evidence bytes; let the compiler finalize the report, derive every digest and byte count from the same final byte objects, create `chatgpt_data_analysis_output.txt`, validate execution/evidence topology, and serialize `audit_return.json` last. Do not change a referenced artifact after it is frozen, and never ask model prose to reproduce hashes, byte counts, Base64, or the full runtime string.
...
- Do not use model-mediated Base64 as the primary proof path. Only after a direct control is unavailable or emits no observable download event, answer the controller's exact one-filename fallback prompt by freshly executing the compiler's `export-wrapper` command for that file. Export `audit_return.json` first, the runtime ledger second, and each remaining output in its own later turn. Return only one strict JSON object in one code block; never reuse metadata or combine files. The controller must bind the exact code-block text bytes and full transport-response capture before parsing. Strictly decoded size and SHA-256 prove only the exported payload actually received, never unavailable download-button bytes; record those original bytes as unresolved, not corrupt.
