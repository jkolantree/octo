...
- Do not use model-mediated Base64 as the primary proof path. Only after a direct control is unavailable or emits no observable download event, answer the controller's exact one-file, one-index fallback prompt by freshly executing the compiler's `export-chunk` command. The compiler compresses the unchanged payload and emits at most 2,048 decoded transport bytes per indexed wrapper. Return only that command's one strict JSON object in one code block; never reimplement it, combine indices or files, or reuse metadata. The controller must bind every exact prompt, complete transport-response capture, code-block text, and parser input; require contiguous indices and one repeated payload/encoded identity; then reassemble and verify the payload. Strict chunk, encoded-stream, and payload checks prove only the exported payload actually received, never unavailable download-button bytes; record those original bytes as unresolved, not corrupt.
