...
- Do not use model-mediated Base64 as the primary proof path. During the original compile transaction, compiler v5 seals exact copies of every exportable non-source output plus `audit_return.json` into a fresh private transport directory from the same final in-memory byte objects; never expose that directory or its contents as file controls or semantic artifacts. Only after a direct control is unavailable or emits no observable download event, answer the controller's exact one-file, one-index fallback prompt by freshly executing the compiler's literal `export-chunk` command against the named private snapshot. The compiler compresses that sealed payload and emits at most 2,048 decoded transport bytes per indexed wrapper. Return only that command's one strict JSON object in one code block; never reimplement it, combine indices or files, reuse metadata, or follow a public output link. The controller must bind every exact prompt, complete transport-response capture, code-block text, and parser input; require contiguous indices and one repeated payload/encoded identity; then reassemble and verify the payload. Strict chunk, encoded-stream, and payload checks prove only the compiler-sealed exported payload actually received, never unavailable download-button bytes; record those original bytes as unresolved, not corrupt.
