...
Observed alpha.10 release, live Update, and post-save verification on 2026-07-28, followed by the alpha.12 engine-mathematics candidate:
...
| Alpha.12 repository package | **unvalidated candidate; not installed live** | The regenerated package documents the exact-Q theorem and release-attestation work. No owner-editor Update or alpha.12 Preview claim is part of this lane |
...
| Repository release line | **v0.3.0-alpha.12** | The exact commit, tree, tag, assets, and keyless attestations are bound by the new tag workflow; alpha.11 is not modified or relabeled |
...
| Public Pages | **deployment verified separately** | [English](https://jkolantree.github.io/octo/) and [Japanese](https://jkolantree.github.io/octo/ja.html) routes must match the merged alpha.12 protocol metadata after Pages deployment |
...
Alpha.10 is the exact tagged package at `99b0804e…` and the separately observed live GPT baseline. Alpha.11 is preserved at `f4d9f43e…`. Alpha.12 uses a new version, exact theorem replay, keyless release attestations, deterministic release gates, and tag for its engine-mathematics bytes. Its regenerated GPT package remains an unvalidated update candidate and is not loaded into the live GPT in this lane. Existing tags and release history must not be rewritten.
