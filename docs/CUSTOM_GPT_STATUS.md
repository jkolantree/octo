...
Observed alpha.10 release, live Update, and post-save verification on 2026-07-28, followed by the alpha.11 engine-mathematics candidate:
...
| Alpha.11 repository package | **unvalidated candidate; not installed live** | The regenerated package documents the engine-mathematics release. No owner-editor Update or alpha.11 Preview claim is part of this lane |
...
| Repository release line | **v0.3.0-alpha.11** | The exact commit, tree, tag, and assets are bound by the new tag and `RELEASE_MANIFEST.json`; alpha.10 is not modified or relabeled |
...
| Public Pages | **deployment verified separately** | [English](https://jkolantree.github.io/octo/) and [Japanese](https://jkolantree.github.io/octo/ja.html) routes must match the merged alpha.11 protocol metadata after Pages deployment |
...
Alpha.10 is the exact tagged package at `99b0804e…` and the separately observed live GPT baseline. Alpha.11 uses a new version, binding record, deterministic release gates, and tag for the engine-mathematics bytes. Its regenerated GPT package remains an unvalidated update candidate and is not loaded into the live GPT in this lane. Existing tags and release history must not be rewritten.
