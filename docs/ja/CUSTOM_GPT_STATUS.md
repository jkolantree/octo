...
2026-07-28 に alpha.10 release/live Update/post-save verification を observe し、その後 alpha.11 engine-mathematics candidate を作成:
...
| Alpha.11 repository package | **unvalidated candidate; live 未導入** | regenerated package は engine-mathematics release を document する。この lane では owner-editor Update も alpha.11 Preview claim も行わない |
...
| Repository release line | **v0.3.0-alpha.11** | exact commit、tree、tag、assets は new tag と `RELEASE_MANIFEST.json` に bind される。alpha.10 を modify/relabel しない |
...
| Public Pages | **deployment は separately verified** | [English](https://jkolantree.github.io/octo/) と [Japanese](https://jkolantree.github.io/octo/ja.html) route は Pages deployment 後に merged alpha.11 protocol metadata と一致する必要がある |
...
## Alpha.8 through alpha.11 boundary
...
Alpha.10 は `99b0804e…` の exact tagged package であり、separately observed live GPT baseline です。Alpha.11 は engine-mathematics bytes に new version、binding record、deterministic release gates、tag を使用します。regenerated GPT package は unvalidated update candidate のままで、この lane では live GPT に load しません。existing tags と release history を rewrite しません。
