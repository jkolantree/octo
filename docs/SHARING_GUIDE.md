...
The v0.3.0-alpha.19 source line is a research preview. Its publication state
...
## v0.3.0-alpha.19 release contents

When present, the annotated `v0.3.0-alpha.19` tag under the project's
...
- `BSC_CUSTOM_GPT_PACKAGE_0.3.0-alpha.19.zip`, the deterministic Custom GPT editor, Knowledge, evaluation, manifest, and checksum package;
- `bsc-audit-engine-0.3.0-alpha.19.zip`, the versioned tracked-source archive;
...
```text
https://github.com/jkolantree/octo/releases/tag/v0.3.0-alpha.19
https://raw.githubusercontent.com/jkolantree/octo/v0.3.0-alpha.19/BSC_AUDIT_LLM_PACKET.md
https://jkolantree.github.io/octo/
https://chatgpt.com/g/g-6a601b1f576881918e659b363ed3063f-bsc-claim-auditor
```

The deployed Japanese Pages URL is `https://jkolantree.github.io/octo/ja.html`.
For each release, verify the English, Japanese, protocol-metadata, and linked
return-schema routes against the exact release commit after Pages deployment;
do not carry a prior route-check date forward.

The repository release, if published, provides a reproducible package—not
proof of the authenticated Custom GPT state. The official GPT's availability,
observable saved state, Preview validation, GitHub release, and Pages
deployment are recorded separately in
...
validation from source wording or a ZIP alone. The alpha.19 generated package
...
account, analytics, or cloud-storage service. Its component contract records
that the byte-identical public protocol remains version `0.3.0-alpha.13`.
...
gh attestation verify PATH/TO/ASSET \
  ...
  --source-ref refs/tags/v0.3.0-alpha.19
```
...
- [ ] The exact tag workflow keylessly attests every final file required by the semantic release roster, manifest, and checksum ledger before publication, and redownloaded bytes verify.
...
The Audit Return Desk was introduced in alpha.8 and remains in alpha.19, under the independently versioned alpha.13 protocol component, for non-admissive inspection of returned output and receipts. Its presence in a Pages or GPT interface does not turn a returned draft into admissible evidence.
