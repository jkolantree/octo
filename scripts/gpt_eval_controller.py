...
def _capture_same_response_transport(
    ...
    *,
    ...
    """Classify and verify the compiler-v9 block in the original response.
    ...
    base: dict[str, Any] = {
        ...
        "detail": "no compiler-v9 stdout block was present in the completed response",
    ...
    if len(identities) > 1:
        ...
        base["detail"] = "more than one compiler-v9 stdout block was present"
        return base, {}
    if partial_candidate is not None:
        base["status"] = "truncated"
        base["detail"] = (
            "the compiler-v9 stdout fenced code block was not complete"
            if partial_fenced
            else "the compiler-v9 stdout was not in a fenced code block"
        )
    ...
    if not block_fenced[candidate_index]:
        base["status"] = "malformed"
        base["detail"] = "the compiler-v9 stdout was not in a fenced code block"
        return base, {}
    trailing_text = [
        text
    ...
    if (
        ...
        base["status"] = "extra"
        base["detail"] = (
            "the compiler-v9 stdout block was not the final response content"
        )
        return base, {}
    try:
    ...
    base.update(
        {
            ...
            "detail": (
                "compiler-v9 same-response transport verified"
                if recovery_receipt["state"] == RECOVERY_STATE_NOT_NEEDED
                else (
                    "compiler-v9 same-response transport verified with "
...
def _parser() -> argparse.ArgumentParser:
    ...
    build = subparsers.add_parser(
        ...
        help=(
            ...
            "compiler-v9 bundle in the original response"
        ),
    )
